...
### PART 02. 실무에 AI Coding 적용하기  

---

## 1. AI Coding을 위한 보안 가이드 🔐
...
### 1.1 AI 활용에서의 보안 문제 ⚠️

#### 1.1.1 AI를 바라보는 관점의 변화: 협업 → 위임 🔄
...
#### 1.1.3 AI 활용 시 발생하는 두 가지 핵심 리스크 🚨
...
### 2.1 회사에서 AI 코딩 도구를 활용하는 세 가지 방식 🏢
...
```markdown
### 3.3 Cursor의 기본 사용 패턴 🧑‍💻

Cursor는 단순 자동완성 도구가 아니라, **개발자의 업무 흐름을 AI와 연결하는 IDE** 입니다.  
기본적으로 다음 세 가지 패턴을 중심으로 활용합니다.
...
- **Inline Edit 📝**  
  ...
  - “이 부분을 함수형으로 바꿔줘” 같은 빠른 수정에 활용  

- **Agent 🤖**  
  - 작업 목표를 주고 전체 흐름을 위임  
  - “로그인 페이지 구현”처럼 큰 단위의 업무를 맡길 수 있음  

👉 이 강의의 핵심은 **Agent 사용** 입니다.  
Agent는 단순히 코드를 제안하는 것이 아니라, **계획 → 실행 → 검토** 흐름을 따릅니다.

---

### 3.4 Rules 설정으로 Cursor를 강화해보자 📏
...
#### 3.4.2 User Rules vs Project Rules ⚖️
...
### 3.5 MCP를 활용해 Cursor를 강화해보자 🔌

Rules가 **기준과 사고방식** 이라면,  
MCP는 **Agent의 손과 발** 입니다.

#### 3.5.1 MCP(Model Context Protocol)란?
...
### 4.1 요구사항을 바탕으로 Cursor 환경 구성하기 🛠️
...
## 6. Cursor에서 Context를 통제하는 방법 🎛️

Cursor의 강점은  
**Context를 ‘눈으로 보며’ 제어할 수 있다는 점** 입니다.

### 6.1 Select – 무엇을 볼 것인가 👀
